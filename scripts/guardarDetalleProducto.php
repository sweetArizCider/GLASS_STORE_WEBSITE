    ...
    if (isset($_POST['producto'], $_POST['alto'], $_POST['ancho'], $_POST['cantidad'], $_POST['total'])) {
        ...
        if (isset($_SESSION["nom_usuario"])) {
            ...
            if ($resultado_ids && !empty($resultado_ids)) {
                ...
                if ($stmt->execute()) {
                    // Cotización guardada con éxito para usuario logueado
                    ?>
                    <!DOCTYPE html>
                    <html lang="es">
                    <head>
            ...
            // Usuario invitado
            $invitado_id = isset($_SESSION["invitado_id"]) ? $_SESSION["invitado_id"] : null;

            if ($invitado_id === null) {
                // Asignar un ID de invitado
                $_SESSION["invitado_id"] = uniqid('invitado_');
                $invitado_id = $_SESSION["invitado_id"];
            }

            $stmt = $pdo->prepare("
                INSERT INTO detalle_producto (producto, invitado_id, alto, largo, cantidad, marco, tipo_cadena, diseno)
                VALUES (:producto, :invitado_id, :alto, :ancho, :cantidad, :marco, :tipo_cadena, :diseno)
            ");
            $stmt->bindParam(':producto', $producto);
            $stmt->bindParam(':invitado_id', $invitado_id);
            $stmt->bindParam(':alto', $alto);
            $stmt->bindParam(':ancho', $ancho);
            $stmt->bindParam(':cantidad', $cantidad);
            $stmt->bindParam(':marco', $color_accesorios);
            ...
            if ($stmt->execute()) {
                // Cotización guardada con éxito para usuario invitado, redirigir a iniciar sesión
                ?>
                <!DOCTYPE html>
                <html lang="es">
                <head>
        ...
        mostrarErrorGuardado('Datos incompletos');
    }
} catch (PDOException $e) {
    mostrarErrorGuardado('Error de base de datos: ' . $e->getMessage());
}

function mostrarErrorGuardado($mensaje = 'Ha ocurrido un error al intentar guardar tu cotización. Por favor, inténtalo de nuevo.') {
    ?>
    <!DOCTYPE html>
    <html lang="es">
    <head>
    ...
    <body>
        <div class="error-container">
            <img src="../img/index/GLASS.png" alt="Glass Store" class="mb-4"style="width: 100px; margin-top:1em;">
            <h1>¡Lo Lamentamos!</h1>
            <p style="margin-bottom:2em;"><?php echo $mensaje; ?></p>
